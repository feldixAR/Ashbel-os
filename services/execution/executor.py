...
# ── Lead Acquisition Handlers (Phase 12) ─────────────────────────────────────

def _handle_discover_leads(task: TaskModel) -> ExecutionResult:
    """Discover leads from a business goal + optional raw signals."""
    from engines.lead_acquisition_engine import run_acquisition
    from services.storage.db import get_session

    started = _now_ms()
    p = _params(task)
    goal    = p.get("goal") or p.get("command") or _command(task) or "לקוחות אלומיניום"
    ...
    try:
        ...
        return ExecutionResult(
            success=True,
            message=(
                f"גילוי לידים הושלם — מטרה: '{goal}'\n"
                f"נמצאו: {result.total_discovered} | חדשים: {result.new_leads} | כפולים: {result.duplicates}"
            ),
            output={
        ...
        return ExecutionResult(success=False, message=f"שגיאה בגילוי לידים: {e}", output={"error": str(e)})


def _handle_process_inbound(task: TaskModel) -> ExecutionResult:
    """Process an inbound lead from Telegram/form/WhatsApp."""
    ...
    try:
        with get_session() as session:
            lead_id = process_inbound(lead_data=p, session=session)
        return ExecutionResult(
            success=True,
            message=f"ליד נכנס נשמר בהצלחה — ID: {lead_id}",
            output={"lead_id": lead_id},
            duration_ms=_elapsed_ms(started),
        )
    except Exception as e:
        return ExecutionResult(success=False, message=f"שגיאה בעיבוד ליד נכנס: {e}", output={"error": str(e)})


def _handle_website_analysis(task: TaskModel) -> ExecutionResult:
    ...
    try:
        ...
        return ExecutionResult(
            success=True,
            message=f"ניתוח אתר — ציון בריאות: {result.audit_score}/100 | לכידת לידים: {result.lead_capture_score}/100",
            output={
                "url":               result.url,
        ...
        return ExecutionResult(success=False, message=f"שגיאה בניתוח אתר: {e}", output={"error": str(e)})
...
def _handle_lead_ops_queue(task: TaskModel) -> ExecutionResult:
    ...
    try:
        ...
        return ExecutionResult(
            success=True,
            message=f"תור פעולות לידים — גולשים: {len(discovered)} | נכנסים: {len(inbound)} | ממתינים: {len(pending_action)}",
            output={
                "discovered": [_lead_dict(l) for l in discovered[:20]],
                "inbound":    [_lead_dict(l) for l in inbound[:20]],
                "pending_action": [_lead_dict(l) for l in pending_action[:20]],
                "counts": {
        ...
        return ExecutionResult(success=False, message=f"שגיאה בטעינת תור לידים: {e}", output={"error": str(e)})
...
_HANDLERS = {
    ...
    "process_inbound":      _handle_process_inbound,
    "website_analysis":     _handle_website_analysis,
    "lead_ops_queue":       _handle_lead_ops_queue,
}
