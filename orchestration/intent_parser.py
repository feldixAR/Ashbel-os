...
# ── Keyword rules ─────────────────────────────────────────────────────────────
# Each rule: (intent, confidence, list-of-trigger-patterns)
# Patterns are checked against the lowercased command with re.search.
# First match wins.

_KEYWORD_RULES: list[tuple[str, float, list[str]]] = [

    # ── Lead / CRM ─────────────────────────────────────────────────────────
    # NOTE: more-specific intents (score, message, followup) must appear
    # BEFORE list_leads to prevent "לידים" from matching prematurely.
...
def _extract_lead_params(cmd: str) -> dict:
    ...
    # ── Structured key=value: שם=X עיר=Y טלפון=Z ─────────────────────────
    ...
    # ── Free-form: "תרשום/הוסף ליד [חדש] NAME מCITY PHONE" ───────────────
...
def _extract_agent_params(cmd: str) -> dict:
    params = {}
    # department from "מחלקת X" or "מחלקה X"
    m = re.search(r"מחלקת?\s+([^\s,]+)", cmd)
    if m:
        params["department"] = m.group(1).strip()
    m = re.search(r"department\s+([^\s,]+)", cmd, re.IGNORECASE)
    if m:
        params["department"] = m.group(1).strip()
    # role/name from "סוכן X" or "agent X"
...
def _extract_content_params(cmd: str) -> dict:
    params = {}
    # topic: everything after "על" / "about" / "בנושא"
    m = re.search(r"(?:על|about|בנושא)\s+(.+?)(?:\s+ל[^\s]|\s+עבור|$)", cmd)
    if m:
        params["topic"] = m.group(1).strip()
    # content type
    for ctype in ["linkedin", "אינסטגרם", "instagram", "בלוג", "blog",
                  "מאמר", "article", "פוסט", "post", "ניוזלטר", "newsletter"]:
        if ctype in cmd:
            params["type"] = ctype
            break
    return params
...
def _extract_topic(cmd: str) -> dict:
    """Generic topic extraction for analysis / brainstorm intents."""
    # Remove leading verb phrase, keep the rest as topic
    cleaned = re.sub(
...
class IntentParser:
    """
    Parses Hebrew (and mixed Hebrew/English) business commands
    into structured IntentResult objects.

    Fast-path:  keyword matching (regex rules above)
    Fallback:   AI-based parsing — wired in Stage 2 Batch 2 when
                model_router is available. Currently returns UNKNOWN
                with confidence=0.0 so the orchestrator can handle gracefully.
    """
    ...
    def parse(self, command: str) -> IntentResult:
        ...
        # AI fallback stub — replaced in Batch 2
